...
import { BarChart3, Loader2, Download, Users, FolderKanban, AlertTriangle, Users2, FileText, ChevronRight, ChevronDown, History } from 'lucide-react'
import { usePermissions } from '@/hooks/usePermissions'
import Pagination from './_pm/Pagination'

const API = '/api'
const AUDIT_LIMIT = 50 // eventos por página na aba Auditoria
...
interface AuditRow { id: string; source: 'task' | 'project' | 'pomodoro'; entity_type: string; entity_id: string; event_type: string; actor_type: string; actor_id: string | null; actor_username: string | null; payload: any; occurred_at: string }
type AuditSource = '' | 'task' | 'project' | 'pomodoro'
...
const RelatoriosTarefas: React.FC = () => {
  ...
  const [tab, setTab] = useState<'productivity' | 'health' | 'teams' | 'audit'>('productivity')
  ...
  // Aba Auditoria (#8): eventos unificados (task/project/pomodoro), filtráveis + paginados.
  const [audit, setAudit] = useState<AuditRow[]>([])
  const [auditSource, setAuditSource] = useState<AuditSource>('')
  const [auditEventType, setAuditEventType] = useState('')
  const [auditPage, setAuditPage] = useState(1)
  const [auditMeta, setAuditMeta] = useState<{ total: number; totalPages: number }>({ total: 0, totalPages: 1 })
  // Qualquer mudança de filtro volta pra página 1 (evita ficar numa página vazia).
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (tab === 'productivity') {
        ...
        const qs = new URLSearchParams({ from, to, limit: String(AUDIT_LIMIT), page: String(auditPage) })
        if (auditSource) qs.set('source', auditSource)
        if (auditEventType.trim()) qs.set('eventType', auditEventType.trim())
        const r = await fetch(`${API}/pm/audit?${qs.toString()}`)
        const j = await r.json(); if (!j.success) throw new Error(j.error)
        setAudit(j.data)
        setAuditMeta({ total: j.pagination?.total ?? j.data.length, totalPages: j.pagination?.totalPages ?? 1 })
      } else {
  ...
  }, [tab, from, to, auditPage, auditSource, auditEventType])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-gray-200 dark:border-gray-700">
        {([['productivity', 'Produtividade', Users], ['teams', 'Equipes', Users2], ['health', 'Saúde dos projetos', FolderKanban], ['audit', 'Auditoria', History]] as const).map(([k, label, Icon]) => (
      ...
      {tab !== 'health' && (
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input type="date" value={from} onChange={e => { setFrom(e.target.value); resetAuditPage() }} className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 dark:text-gray-100 text-sm" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Até</label>
            <input type="date" value={to} onChange={e => { setTo(e.target.value); resetAuditPage() }} className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 dark:text-gray-100 text-sm" />
          </div>
          <div className="flex gap-1">
            {[['7d', 7], ['30d', 30], ['90d', 90]].map(([l, n]) => (
              <button key={l as string} onClick={() => { setFrom(daysAgoISO(n as number)); setTo(todayISO()); resetAuditPage() }}
          ...
          {tab === 'audit' && (
            <>
              <div>
                ...
                <select value={auditSource} onChange={e => { setAuditSource(e.target.value as AuditSource); resetAuditPage() }}
                  className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 dark:text-gray-100 text-sm">
              ...
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Tipo de evento</label>
                <input type="text" value={auditEventType} placeholder="ex.: completed" onChange={e => { setAuditEventType(e.target.value); resetAuditPage() }}
                  className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 dark:text-gray-100 text-sm w-40" />
              </div>
            </>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-[#2d3f52] text-gray-600 dark:text-gray-300">
                <tr>
                  ...
                  <th className="text-left px-4 py-2 font-semibold">Fonte</th>
                  <th className="text-left px-4 py-2 font-semibold">Evento</th>
                  <th className="text-left px-4 py-2 font-semibold">Entidade</th>
                  <th className="text-left px-4 py-2 font-semibold">Ator</th>
                  <th className="text-left px-4 py-2 font-semibold">Detalhes</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {audit.length === 0 && <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-400">Sem eventos no período/filtro.</td></tr>}
                {audit.map(a => {
                  const badge = a.source === 'task' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300'
                    : a.source === 'project' ? 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300'
                    : 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300'
                  const payloadStr = a.payload && Object.keys(a.payload).length ? JSON.stringify(a.payload) : ''
                  return (
                    <tr key={a.id} className="text-gray-800 dark:text-gray-100 align-top">
                      <td className="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400">{new Date(a.occurred_at).toLocaleString('pt-BR')}</td>
                      <td className="px-4 py-2"><span className={`text-[10px] font-medium uppercase tracking-wide px-1.5 py-0.5 rounded ${badge}`}>{a.source}</span></td>
                      <td className="px-4 py-2 font-medium">{a.event_type}</td>
                      <td className="px-4 py-2 text-xs text-gray-500 dark:text-gray-400 font-mono max-w-[10rem] truncate" title={a.entity_id || ''}>{a.entity_id || '—'}</td>
                      <td className="px-4 py-2 text-xs">{a.actor_username || (a.actor_type === 'user' ? (a.actor_id || '—') : a.actor_type)}</td>
                      <td className="px-4 py-2 text-xs text-gray-400 font-mono max-w-[16rem] truncate" title={payloadStr}>{payloadStr}</td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <Pagination page={auditPage} totalPages={auditMeta.totalPages} total={auditMeta.total} onPage={setAuditPage} disabled={loading} />
        </div>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
          <table className="w-full text-sm">
