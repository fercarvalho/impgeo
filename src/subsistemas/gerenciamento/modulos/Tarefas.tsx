...
import { ListTodo, Play, Pause, RotateCcw, CheckCircle2, Clock, Loader2, AlertTriangle, X, HelpCircle, UserPlus, Inbox, Timer, CalendarClock, Check, Users, Undo2 } from 'lucide-react'
import { usePermissions } from '@/hooks/usePermissions'
import { useDialogs } from '@/components/DialogProvider'
import PendingTasksBanner from './_pm/PendingTasksBanner'
import {
  fetchMyTasks, taskAction, TASK_STATUS_META, PmTask,
  fetchIncomingHelp, helpAction, HelpRequest,
  fetchAvailableTasks,
  DueDateRequest,
...
const Tarefas: React.FC = () => {
  ...
  // Tarefas disponíveis para pegar — paginada (melhoria #12). As filas de
  // aprovação (prazo/reabertura/delegação/revisão) migraram para a Central de
  // Aprovações (#11); aqui fica só o pessoal do usuário.
  const PAGE = 25
  const availableList = usePaginatedList<PmTask>(fetchAvailableTasks, PAGE)
  const available = availableList.items

  // Recarrega a lista paginada via ref, para o `load` (deps estáveis) poder
  // dispará-la sem se recriar e sem re-armar os efeitos.
