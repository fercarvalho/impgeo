...
pg_dump impgeo_db > /var/backups/impgeo/pre-subsistemas-$(date +%Y%m%d-%H%M).sql

# 1. Subsistemas + ALTER modules_catalog + renomeio das 3 chaves antigas
psql -d impgeo_db -v ON_ERROR_STOP=1 -f server/migrations/016-SUBSISTEMAS.sql

# 2. Permissões dos 4 módulos novos do gerenciamento para usuários existentes
psql -d impgeo_db -v ON_ERROR_STOP=1 -f server/migrations/017-PERMISSOES-NOVOS-MODULOS-GERENCIAMENTO.sql
```
...
```bash
psql -d impgeo_db -v ON_ERROR_STOP=1 -f server/migrations/017-PERMISSOES-NOVOS-MODULOS-GERENCIAMENTO-rollback.sql
psql -d impgeo_db -v ON_ERROR_STOP=1 -f server/migrations/016-SUBSISTEMAS-rollback.sql
```
...
psql -d impgeo_db -f server/migrations/017-PERMISSOES-NOVOS-MODULOS-GERENCIAMENTO-rollback.sql
psql -d impgeo_db -f server/migrations/016-SUBSISTEMAS-rollback.sql
...
psql -d impgeo_db < /var/backups/impgeo/pre-subsistemas-<timestamp>.sql
```
